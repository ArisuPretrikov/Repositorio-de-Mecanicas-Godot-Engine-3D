...
class MatchManager {
    ...
    // ─── Message handlers ─────────────────────────────────────────────────────────
    handleQuickJoin(player, rawName) {
    ...
    handleCreateLobby(player, rawName) {
        if (player.state !== 'idle' && player.state !== 'spectating') {
            this.send(player.id, { type: 'error', message: 'Você já está em uma partida ou na fila.' });
            return;
        }
    ...
    handleJoinLobby(player, rawName, rawCode) {
        if (player.state !== 'idle' && player.state !== 'spectating') {
            this.send(player.id, { type: 'error', message: 'Você já está em uma partida ou na fila.' });
            return;
        }
    ...
    handleCheckpointPassed(player, checkpointIndex) {
    ...
    handleLapCompleted(player, lap, elapsedMs) {
        if (player.state !== 'racing' || !player.matchId) {
            this.send(player.id, { type: 'error', message: 'Você não está em uma corrida.' });
            return;
        }
        if (typeof lap !== 'number' || lap < 1 || lap > 100 ||
    ...
    handleRaceFinished(player, totalMs) {
        if (player.state !== 'racing' || !player.matchId) {
            this.send(player.id, { type: 'error', message: 'Você não está em uma corrida.' });
            return;
        }
        ...
        while (this.quickQueue.length >= 2) {
            ...
            if (!p1 || p1.state !== 'queued') {
                if (p2 && p2.state === 'queued')
                    this.quickQueue.unshift(p2Id);
                continue;
            }
            if (!p2 || p2.state !== 'queued') {
                if (p1 && p1.state === 'queued')
                    this.quickQueue.unshift(p1Id);
                continue;
            }
            p1.state = 'racing';
            p1.playerNumber = 1;
    ...
    endMatch(matchId, forcedWinnerId) {
        const match = this.matches.get(matchId);
        if (!match || match.state === 'finished')
            return;
    ...
    notifySpectators(matchId, event) {
        const match = this.matches.get(matchId);
        if (!match)
            return;
        for (const specId of match.spectatorIds) {
            this.send(specId, event);
        }
    }
    // ─── Utilities ────────────────────────────────────────────────────────────────
    send(playerId, msg) {
    ...
    removeFromAllSpectate(playerId) {
        for (const match of this.getUniqueMatches()) {
